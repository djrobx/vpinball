...
void Player::DrawBalls()
{
    ...
    const float cs = cosf(inclination);
        ...
        // sort balls by depth to avoid anti-aliasing issues
        std::vector<std::pair<float,unsigned int> > sorted_balls(m_vball.size());
    ...
    for (unsigned i=0; i<m_vball.size(); i++)
    {
      ...
      float minz = pball->defaultZ-1.0f;
                ...
                const float wp = m_pin3d.m_proj.m_matrixTotal._14*m_vball[i]->pos.x + m_pin3d.m_proj.m_matrixTotal._24*m_vball[i]->pos.y + m_pin3d.m_proj.m_matrixTotal._34*zheight + m_pin3d.m_proj.m_matrixTotal._44;

                sorted_balls[i] = std::pair<float,unsigned int>(wp, i);
        }

        std::sort(sorted_balls.begin(), sorted_balls.end());

    for (unsigned itmp=0; itmp<m_vball.size(); itmp++)
    {
        Ball * const pball = m_vball[sorted_balls[m_vball.size()-1-itmp].second]; // draw back to front
        // just calculate the vertices once!
        float zheight = (!pball->fFrozen) ? pball->pos.z : (pball->pos.z - pball->radius);
